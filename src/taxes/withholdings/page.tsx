  ...
  return (
    <div style={S.page}>
      <header style={S.header}>
        <div>
          <Link href="/" style={{ color: '#64748b', textDecoration: 'none', fontSize: '0.85rem' }}>← Dashboard</Link>
          <h1 style={{ fontSize: '1.5rem', margin: '0.25rem 0 0', color: '#1e293b' }}>Retenciones</h1>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <Link href="/taxes/rules" style={{ padding: '0.6rem 1rem', color: '#64748b', textDecoration: 'none', fontSize: '0.9rem' }}>Reglas →</Link>
          <Link href="/taxes/ats" style={{ padding: '0.6rem 1rem', color: '#64748b', textDecoration: 'none', fontSize: '0.9rem' }}>ATS →</Link>
          <button style={S.btn} onClick={() => setModalOpen(true)}>+ Nueva Retención</button>
        </div>
      </header>
              ...
              <tbody>
                {withholds.map(w => (
                  <tr key={w.id}>
                    ...
                    <td style={S.td}>{badge(w.state)}</td>
                    <td style={S.td}>{w.state === 'draft' && <button style={S.btnSm} onClick={() => handlePost(w.id)}>Emitir</button>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
