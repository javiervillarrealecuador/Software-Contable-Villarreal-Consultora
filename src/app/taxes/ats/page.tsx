...
export default function AtsPage() {
  ...
  const [stats, setStats] = useState<{ compras: number; ventas: number; totalVentas: number } | null>(null);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setXml(x);
      setStats({ compras: data.compras.length, ventas: data.ventas.length, totalVentas: data.totalVentas });
    ...
    <div style={S.page}>
      ...
      <main style={S.main}>
        <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '0.5rem', padding: '0.9rem 1.2rem', marginBottom: '1.5rem', fontSize: '0.88rem', color: '#92400e' }}>
          Compras: retenciones <strong>emitidas</strong> del período.
          Ventas: facturas <strong>entregadas</strong> (CAPA 4), agrupadas por cliente como exige el SRI.
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <div>
                ...
                <span style={{ marginLeft: '1rem', color: '#64748b', fontSize: '0.88rem' }}>
                  {stats?.compras || 0} compras con retención · {stats?.ventas || 0} grupos de venta · ventas ${(stats?.totalVentas || 0).toFixed(2)} en {MESES[mes - 1]} {anio}
                </span>
              </div>
